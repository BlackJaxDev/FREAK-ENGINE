    ...
    public class Model : IDisposable
    {
        ...
        private unsafe Mesh ProcessMesh(Silk.NET.Assimp.Mesh* mesh, Scene* scene)
        {
            ...
            for (uint i = 0; i < mesh->MNumVertices; i++)
            {
                ...
                Vector3 vector = new(); // we declare a placeholder vector since assimp uses its own vector class that doesn't directly convert to glm's vec3 class so we transfer the data to this placeholder glm::vec3 first.
                                                // positions
