...
namespace XREngine.Data.Transforms.Rotations
{
    ...
    public unsafe struct Rotator
    {
        ...
        {
            Quaternion p = Quaternion.CreateFromAxisAngle(Vector3.UnitX, XMath.DegToRad(Pitch));
            Quaternion y = Quaternion.CreateFromAxisAngle(Vector3.UnitY, XMath.DegToRad(Yaw));
            Quaternion r = Quaternion.CreateFromAxisAngle(Vector3.UnitZ, XMath.DegToRad(Roll));
            return Order switch
            {
        ...
        public readonly Quaternion GetYawQuaternion()
            => Quaternion.CreateFromAxisAngle(Globals.Up, XMath.DegToRad(Yaw));
        public readonly Quaternion GetPitchQuaternion()
            => Quaternion.CreateFromAxisAngle(Globals.Right, XMath.DegToRad(Pitch));
        public readonly Quaternion GetRollQuaternion()
            => Quaternion.CreateFromAxisAngle(Globals.Forward, XMath.DegToRad(Roll));

        public static Rotator CreateLookat(Vector3 vector)
            => XMath.LookatAngles(vector);
