    ...
    public class DeferredRenderPipeline : XRRenderPipeline
    {
        ...
        public override unsafe void InitializeFBOs()
        {
            ...
            //if (Engine.Settings.EnableDeferredPass)
            {
                ...
                Models.Materials.BlendMode additiveBlend = new()
                {
        ...
        public override void Render(VisualScene visualScene, XRCamera camera, XRViewport? viewport, XRFrameBuffer? target)
        {
            //_timeQuery.BeginQuery(EQueryTarget.TimeElapsed);
            Engine.Rendering.State.PushRenderingCamera(camera);
            try
            {
                if (viewport != null)
                    RenderToViewport(visualScene, camera, viewport, target);
                else
                    RenderToFBO(target);
            }
            catch (Exception e)
        ...
        private void RenderToFBO(XRFrameBuffer? target)
        {
            ...
            _renderCommands.Render((int)ERenderPass.Background);

            Engine.Rendering.State.AllowDepthWrite(true);
            _renderCommands.Render((int)ERenderPass.OpaqueDeferredLit);
            _renderCommands.Render((int)ERenderPass.OpaqueForward);
            _renderCommands.Render((int)ERenderPass.TransparentForward);
            ...
            _renderCommands.Render((int)ERenderPass.OnTopForward);
        ...
        private void RenderToViewport(VisualScene visualScene, XRCamera camera, XRViewport? viewport, XRFrameBuffer? target)
        {
            PushRenderingCamera(camera);
            {
                ...
                using (Engine.Rendering.State.PushRenderArea(viewport.Region))
                {
                    ...
                    target?.BindForWriting();
                    PostProcessFBO!.RenderFullscreen();
                    target?.UnbindFromWriting();
                }
            }
            PopRenderingCamera();
        ...
        private void RenderDeferredPass()
        {
            using (SSAOFBO!.BindForWriting())
            {
                ...
                Engine.Rendering.State.ClearDepth(1.0f);
                _renderCommands.Render((int)ERenderPass.OpaqueDeferredLit);
                _renderCommands.Render((int)ERenderPass.DeferredDecals);
